...
- 支持两种 payload 形式：
  - 单条：站点识别字段 + `name` + `api_base_url` + `group_name` + 可选 `api_key` / `canonical_model_name` / `request_model_name`
  - 多条：站点识别字段 + `probe_apis: [{name, api_base_url, group_name, api_key?, canonical_model_name?, request_model_name?, notes?}]`
