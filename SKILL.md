...
- `充值比` 是站点级字段，缺省为 `1:1`
- 用户只要求调整分组、倍率、API Key、分组备注或分组启用状态时，禁止默认写入或覆盖充值比；必须先只读确认并保留站点/价格记录当前充值比。
...
- 用户说“添加分组 / 增加分组 / 新增分组 / 调整新分组倍率”时，先只读核对 `mpc_station_group_multipliers` 与 `mpc_price_records` 是否已有对应 `station_id + group_name`：
  - 已有分组且已有价格记录：先用 `update-station` 保留完整分组映射，再对已有模型记录逐条执行 `patch-record` 同步倍率。
  - 新分组且该站已有 `gpt-5.4` / `gpt-5.5` 价格记录：先用 `update-station` 保留完整分组映射，再用官方模型默认价为同站同模型执行 `upsert` 创建价格记录；补录 payload 必须显式带入该站当前充值比或同站同模型既有记录充值比，不能省略后让脚本回退为 `1:1`。`gpt-5.4` 默认价为输入 `2.50 / 1M`、输出 `15.00 / 1M`、缓存读 `0.25 / 1M`，`gpt-5.5` 默认价为输入 `5.00 / 1M`、输出 `30.00 / 1M`、缓存读 `0.50 / 1M`。
  - 新分组但模型默认价无法命中，或该站没有可参考的现有模型记录：不要只写站点级分组表，先提示缺少模型/价格信息。
  - 最终回复必须说明本次是否创建或同步了 `mpc_price_records`，并说明充值比保持值；若只写入站点级分组配置，必须明确提示该分组暂不参与展示、排序和探测。
